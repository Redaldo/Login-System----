...
if(isset($_GET['register'])) {
	...
	if(empty($firstname) || empty($nachname) || empty($email)) {
	...
	//Keine Fehler, wir können den Nutzer Register 
	if(!$error) {	
		$passwort_hash = password_hash($passwort, PASSWORD_DEFAULT);
		
		$statement = $pdo->prepare("INSERT INTO users (email, passwort, firstname, nachname) VALUES (:email, :passwort, :firstname, :nachname)");
		$result = $statement->execute(array('email' => $email, 'passwort' => $passwort_hash, 'firstname' => $firstname, 'nachname' => $nachname));
		
		if($result) {		
			echo 'You have been registered successfully.. <a href="login.php">Login</a>';
			$showFormular = false;
		} else {
...
<input type="text" id="inputFirstname" size="40" maxlength="250" name="firstname" class="form-control" required>
</div>
